...
- [ ] **[both]** Создать репо `Planetary-Ocean-Simulator`, добавить `CLAUDE.md`, `README.md`, `TASKS.md`
- [ ] **[both]** Согласовать ветки: `main` защищён, фичи через `feat/FE-*`, `feat/BE-*`, `feat/PHY-*`
- [ ] **[V]** Скаффолдить `backend/` — FastAPI, пустые роутеры, `uvicorn main:app --reload` запускается без ошибок
...
- [ ] **[V]** `Globe.tsx` — CesiumJS viewer, тёмный фон `#050d1a`, без стандартного imagery
...
- [ ] **[V]** Timeline slider внизу (t: 0 → T_total_steps), кнопка play/pause
- [ ] **[V]** `useOceanState` hook с throttle 100ms — хитмап обновляется при движении слайдера
- [ ] **[V]** Loading indicator во время fetch
- [ ] **[V]** Control panel — 4 слайдера:
